  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">{t("Daily Summary")}</h1>
      ...
      {todayCashMovements.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">{t("Cash movements today")}</CardTitle>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          ...
          hint={
            language === "si"
              ? `−${formatLKR(roomExpenses)} කාමර වියදම්`
        ...
        <StatCard
          ...
          hint={
            language === "si"
              ? `−${formatLKR(restaurantExpenses)} රෙස්ටුරන්ට් වියදම්`
              : `−${formatLKR(restaurantExpenses)} restaurant expenses`
          }
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">{t("Room sales — checkouts today")}</CardTitle>
        </CardHeader>
                ...
                <TableRow key={i}>
                  ...
                  <TableCell>
                    <Badge variant={r.paymentMethod === "bank_transfer" ? "warning" : "secondary"}>
                      {t(PAYMENT_LABEL[r.paymentMethod])}
                    </Badge>
                  </TableCell>
              ...
              {roomSales.length === 0 && (
                <TableRow>
                  <TableCell colSpan={5} className="py-8 text-center text-sm text-muted-foreground">
                    {t("No checkouts recorded for this date.")}
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
      ...
      <Card>
        <CardHeader className="flex-row items-center justify-between space-y-0">
          <CardTitle className="text-base">{t("Item sales — restaurant / POS")}</CardTitle>
          <span className="text-sm text-muted-foreground">
            {language === "si"
              ? `උප එකතුව ${formatLKR(posSubtotal)} · සේවා ගාස්තුව ${formatLKR(posServiceCharge)}`
              ...
              {itemSales.length === 0 && (
                <TableRow>
                  <TableCell colSpan={3} className="py-8 text-center text-sm text-muted-foreground">
                    {t("No completed orders for this date.")}
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
              ...
              {expenses.length === 0 && (
                <TableRow>
                  <TableCell colSpan={4} className="py-8 text-center text-sm text-muted-foreground">
                    {t("No expenses logged for this date.")}
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
      ...
      {creditAccountBalances.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">{t("Credit accounts — still owing")}</CardTitle>
            <p className="text-xs text-muted-foreground">
              {t(
          ...
          <CardContent className="space-y-3">
            ...
            {creditSales.length > 0 && (
              <div className="space-y-1.5 border-t pt-3">
                <p className="text-xs font-medium text-muted-foreground">{t("Added today")}</p>
                {creditSales.map((c, i) => (
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="rounded-lg border p-3">
            <p className="text-sm font-medium">{t("Restaurant")}</p>
            <div className="mt-2 space-y-1.5 text-sm">
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">{t("Revenue")}</span>
    ...
    <Card>
      ...
      <CardContent className="space-y-1.5">
        <div className="flex items-center justify-between text-sm">
          <span className="text-muted-foreground">{t("Inhand (yesterday's closing)")}</span>
