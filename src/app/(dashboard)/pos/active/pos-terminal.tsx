...
const TABLE_STYLES: Record<TableStatus, string> = {
  vacant: "border-emerald-500/40 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400",
  occupied: "border-sky-500/40 bg-sky-500/10 hover:bg-sky-500/20 text-sky-600 dark:text-sky-400",
  reserved: "border-violet-500/40 bg-violet-500/10 hover:bg-violet-500/20 text-violet-600 dark:text-violet-400",
  billed: "border-amber-500/40 bg-amber-500/10 hover:bg-amber-500/20 text-amber-600 dark:text-amber-400",
};
    ...
    <div className="grid gap-6 xl:grid-cols-5">
      ...
      <div className="sticky top-0 z-10 -mt-4 space-y-3 bg-background pb-3 pt-4 md:-mt-6 md:pt-6">
          ...
          <TabsContent value="dine_in" className="space-y-2">
              ...
              {tables.map((table) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex min-h-[4.5rem] flex-col items-center justify-center gap-1 rounded-xl border-2 px-2 py-2.5 text-center transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-60",
                  ...
                  >
                    ...
                    {order ? (
                      ...
                      <span className="text-[10px] font-medium uppercase leading-none tracking-wide opacity-70">
            ...
            {tables.length > 0 && (
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 pt-0.5 text-xs text-muted-foreground">
                {(Object.keys(TABLE_DOT) as TableStatus[]).map((s) => (
                  <span key={s} className="flex items-center gap-1.5 capitalize">
                    <span className={cn("h-2 w-2 rounded-full", TABLE_DOT[s])} />
                    {s}
                  </span>
                ))}
              </div>
          ...
          {menuQuery.trim() === "" && (
            <div className="flex flex-wrap gap-1.5">
              {categories.map((c) => (
                ...
                >
                  {c.station === "bar" ? <Beer className="h-3.5 w-3.5" /> : <ChefHat className="h-3.5 w-3.5" />}
                  {c.name}
                </button>
              ))}
            </div>
        ...
        <div className="rounded-lg border p-3">
          <div className="grid grid-cols-2 content-start gap-3 sm:grid-cols-3 lg:grid-cols-4">
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    inCart ? "border-primary/60 ring-1 ring-primary/30" : "hover:-translate-y-0.5 hover:border-primary/50 hover:shadow-md",
                ...
                >
                  <div
                    ...
                    className="relative aspect-[4/3] w-full cursor-pointer bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset"
                  >
                    {m.image_url ? (
                    ...
                    {inCart && (
                      <span className="absolute left-1.5 top-1.5 flex h-6 min-w-6 items-center justify-center rounded-full bg-primary px-1.5 text-xs font-bold text-primary-foreground shadow-sm">
                        {cardQty}
                      </span>
                    )}
                  </div>
                  <div className="flex flex-1 flex-col justify-between gap-2 p-2.5">
                    ...
                    {/* Always-visible (not hover-only, so it works on
                        touchscreens too) qty adjuster — only takes up space
                        once the item is actually on the order. */}
                    {inCart && (
                      <div className="flex items-center justify-between gap-1 rounded-lg border bg-muted/50 p-1">
                        <button
                          ...
                          className="flex h-7 w-7 items-center justify-center rounded-md bg-background text-foreground shadow-sm transition-colors hover:bg-accent disabled:pointer-events-none disabled:opacity-40"
                        >
                        ...
                        {editingQtyId === m.id ? (
                          ...
                          <button
                            ...
                            disabled={disabled}
                            onClick={() => openQtyEdit(m.id)}
                            className="min-w-8 rounded-md px-1 text-center text-sm font-semibold tabular-nums hover:bg-background disabled:pointer-events-none"
                          >
                            {cardQty}
                          </button>
                        )}
                        <button
                          ...
                          className="flex h-7 w-7 items-center justify-center rounded-md bg-primary text-primary-foreground shadow-sm transition-colors hover:opacity-90 disabled:pointer-events-none disabled:opacity-40"
                        >
              ...
              <>
                            ...
                            <div className="flex items-center gap-0.5 rounded-md border">
                              ...
                              {editingLineId === item.id ? (
                                <input
                                  ...
                                  className="num h-7 w-10 rounded-md border bg-background text-center text-xs font-semibold outline-none focus-visible:ring-2 focus-visible:ring-ring"
                                />
                              ) : (
                                <button
                                  type="button"
                                  ...
                                  className="min-w-5 rounded px-0.5 text-center text-xs font-semibold tabular-nums hover:bg-accent disabled:pointer-events-none"
                                >
                                  {item.quantity}
                                </button>
                              )}
                ...
                <div className="space-y-2 border-t pt-3">
                  ...
                  {dateConfirmId === selectedOrder.id ? null : selectedOrder.channel_type === "room_service" ? (
                    ...
                    <>
                      <div>
                        <Label className="mb-1.5 block text-xs text-muted-foreground">Paid by</Label>
