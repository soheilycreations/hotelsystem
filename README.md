...
- **Room vs Restaurant P&L split** — the expense form now has an **"Allocate to"** field (Restaurant or Room; defaults to Restaurant, matching how expenses were logged before this existed). The P&L Report ends with a **Room vs Restaurant** section: two side-by-side balance cards (Revenue − Expenses) plus a comparison chart, so each side of the business can be judged on its own — owner bank-transfer expenses are excluded from both, same rule as the overall Net Profit.
