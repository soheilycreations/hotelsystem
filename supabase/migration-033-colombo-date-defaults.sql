-- Every "today" date default in this schema used plain `current_date`,
-- which evaluates in the DATABASE's own session timezone (UTC on
-- Supabase) — not Sri Lanka's (UTC+5:30). Any row inserted with no
-- explicit date, between 00:00 and 05:29 Colombo time, silently landed on
-- YESTERDAY's date instead of today's (e.g. a table opened at 12:30am
-- got business_date = the day before). Switching the default to compute
-- the calendar date in Asia/Colombo fixes this everywhere it can happen:
-- opening a bill, logging an expense/cash movement, recording a credit
-- repayment/adjustment, or entering a purchase with no date picked.

alter table public.restaurant_orders
  alter column business_date set default ((now() at time zone 'Asia/Colombo')::date);

alter table public.expenses
  alter column date set default ((now() at time zone 'Asia/Colombo')::date);

alter table public.cash_movements
  alter column date set default ((now() at time zone 'Asia/Colombo')::date);

alter table public.credit_repayments
  alter column date set default ((now() at time zone 'Asia/Colombo')::date);
...
alter table public.purchases
  alter column purchase_date set default ((now() at time zone 'Asia/Colombo')::date);

-- Same fix for rpc_record_purchase's fallback when no date is passed in.
create or replace function public.rpc_record_purchase(
  ...
  p_division       expense_division default 'restaurant'
)
returns uuid
language plpgsql
